...
class MariaMusicSettingRepository extends MusicSettingRepository {
    ...
    public <S extends MusicSetting> boolean create(@NotNull S entity) {
        return this.execute(
                "INSERT INTO `music_setting` (channel_id, volume, `repeat`, show_np, restrict_channel) VALUES (?, ?, ?, ?, ?)",
                entity.getChannelId(),
                entity.getVolume(),
                entity.getRepeat(),
                entity.isShowNp() ? 1 : 0,
    ...
    public boolean exists(@NotNull MusicSettingId musicSettingId) {
        ResultSet res = this.executeQuery(
                "SELECT COUNT(*) FROM `music_setting` WHERE `channel_id` = ?",
                musicSettingId.getChannelId()
        );
    ...
    public MusicSetting findOne(@NotNull MusicSettingId musicSettingId) {
        ResultSet res = this.executeQuery(
                "SELECT * FROM `music_setting` WHERE `channel_id` = ?",
                musicSettingId.getChannelId()
        );
    ...
    public boolean update(@NotNull MusicSetting entity) {
        return this.execute(
                "UPDATE `music_setting` SET volume = ?, `repeat` = ?, show_np = ?, restrict_channel = ? WHERE `channel_id` = ?",
                entity.getVolume(),
                entity.getRepeat(),
                entity.isShowNp() ? 1 : 0,
                entity.getRestrictChannel(),
    ...
    public boolean delete(@NotNull MusicSettingId musicSettingId) {
        return this.execute(
                "DELETE FROM `music_setting` WHERE `channel_id` = ?",
                musicSettingId.getChannelId()
        );
    }
}
