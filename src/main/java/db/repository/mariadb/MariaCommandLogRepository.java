...
class MariaCommandLogRepository extends CommandLogRepository {
    ...
    protected CommandLog bind(@NotNull ResultSet res) throws SQLException {
        return new CommandLog(res.getInt(1), res.getString(2), res.getString(3),
                res.getString(4), res.getString(5), res.getTimestamp(6));
    }

    @Override
    public <S extends CommandLog> boolean create(@NotNull S entity) {
        return this.execute(
                "INSERT INTO `command_log` (kind, full, channel_id, user_id, created_at) VALUES (?, ?, ?, ?, ?)",
                entity.getKind(),
                entity.getFull(),
                entity.getChannelId(),
                entity.getUserId(),
    ...
    public boolean update(@NotNull CommandLog entity) {
        return this.execute(
                "UPDATE `command_log` SET `kind` = ?, `full` = ?, `channel_id` = ?, `user_id` = ?, `created_at` = ? WHERE `id` = ?",
                entity.getKind(),
                entity.getFull(),
                entity.getChannelId(),
                entity.getUserId(),
